    ...
    return (
        <section className={`py-16 w-full flex justify-center bg-[#710000] font-mulish ${!isBackground && "bg-transparent"}`}>
            <div className="w-10/12 lg:w-9/12 lg:px-20 flex flex-col gap-11">
                <h6 className={`text-center text-[24px] lg:text-4xl font-extrabold ${isBackground ? "text-[#FF9090]" : "text-[#040000]"}`} data-aos="fade-up">Businesses worldwide trust our expertise to elevate and grow their brand.</h6>
                <div className="flex justify-between gap-x-6 md:gap-x-[50px] w-full">
                    {counter.map((item, index) => (
                        <div key={item.id} className="flex flex-col gap-y-[7px] items-center justify-center text-center" data-aos="zoom-in" data-aos-delay={100 * index}>
                ...
                <div className={`border-t border-t-[#DCDCDC] pt-8 flex flex-col items-center gap-y-4 ${!isBackground && "hidden"}`} data-aos="fade-up" data-aos-delay="400">
